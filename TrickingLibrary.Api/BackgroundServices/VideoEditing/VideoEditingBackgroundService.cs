            ...
            while (await _channelReader.WaitToReadAsync(stoppingToken))
            {
                ...
                try
                {
                    var starInfo = new ProcessStartInfo
                    {
                        ...
                        Arguments =
                            $"-y -i {inputPath} -an -vf scale=540x380 {outputConvertedPath} -ss 00:00:00 -vframes 1 -vf scale=540x380 {outputThumbnailPath}",
                        CreateNoWindow = true,
                        UseShellExecute = false
                    };

                    using (var process = new Process {StartInfo = starInfo})
                    {
                        process.Start();
                        process.WaitForExit();
                    }
                    ...
                    if (!_videoManager.TemporaryFileExists(outputThumbnailName))
                        throw new Exception("FFMPEG Failed to create thumbnail");
                    ...
                    using (var scope = _serviceProvider.CreateScope())
                    {
                        ...
                        var submission = await ctx.Submissions
                            .FirstOrDefaultAsync(x => x.Id.Equals(message.SubmissionId));
